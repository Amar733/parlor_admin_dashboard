...
const TemplateNineteen: React.FC<PrescriptionTemplateProps> = ({
  doctor,
  patient,
  notes,
  rows,
...
>
  ...
  {/* Header */}
  <header className="bg-gray-100 border-t border-b-4 border-black border-teal-600 py-8 px-6 mb-4 pointer-events-auto text-black md:px-10 lg:px-16"
   style={{ fontFamily: fontStyle, color: textColor, transform: `scale(${fontSize / 100})`, transformOrigin: 'top left', width: `${100 / (fontSize / 100)}%` }}
  >
  ...
  {/* Patient Info Grid */}
 <section className="grid grid-cols-3 gap-y-2 gap-x-4 text-sm border-b border-gray-200 pb-4 mb-8 pointer-events-auto">
  {/* Sr. No./Date combined - stacked */}
  <div className="flex flex-col ml-4">
    <strong className="text-gray-900">Sr. No./Date:</strong>
...
<div className="grid grid-rows-2 items-start gap-4">
  ...
  <div className="flex items-center gap-1">
    <PrintableInput 
      className="w-12 bg-transparent border-b border-gray-300 focus:border-teal-500 outline-none  text-center px-0.5 py-0 ml-16"
      value={patient.age}
    ...
    <PrintableInput 
      className="w-16 bg-transparent border-b border-gray-300 focus:border-teal-500 outline-none  text-center px-0.5 py-0 -ml-6 "
      value={patient.sex}
  ...
  {rows.map((row, i) => {
    ...
    return (
      ...
      >
        ...
        <div className="relative flex items-start justify-center p-1 border-r border-black min-h-[44px]">
          <PrintableTextarea 
            className={`text-xs  text-center w-full ${activeTool === 'text' ? 'z-20 cursor-text' : 'z-0'}`}
...
{/* </div> */}
  ...
  {/* Footer */}
  <div className="w-full border-b-4  border-gray-900 mb-0">
 ...
 <div className="mt-auto pt-8 flex flex-col items-end pointer-events-auto bg-gray-100">
  ...
  <p className="text-sm font-bold text-black mr-3">{doctor.name} (Signature)</p>
