...
const TemplateThree: React.FC<PrescriptionTemplateProps> = ({
  doctor,
  patient,
  notes,
  rows,
  ...
  return (
    <div 
     ...
     >
            ...
            {/* Header */}
     <header className="bg-orange-200 border-b-4 border-orange-400 pb-4 mb-4 pointer-events-auto text-black pl-6 md:pl-8 lg:pl-10"
    ...
    <div>
      ...
      <p className="text-xs-1 mb-1 opacity-90">{doctor.speciality} | Reg No: <strong>{doctor.regNo}</strong></p>
      <p className="text-xs-1 mb-0 opacity-90">{doctor.address}</p>
      <h1 className="text-2xl font-bold uppercase tracking-wider">{doctor.clinicName}</h1>
      <p className="text-xs-1 opacity-90"><strong>Email:</strong> {doctor.email} | <strong>Phone:</strong> {doctor.phone}</p>
    </div>
  </div>
</header>
         
            {/* Patient Info Grid */}
           <section className="grid grid-cols-1 gap-4 text-sm border-b border-gray-200 pb-4 mb-4 pointer-events-auto">
  {/* First Row: Three boxes in one row */}
  <div className="grid grid-cols-3 gap-4">
    {/* Box 1: Sr.No + Date */}
    <div className="border border-gray-300 rounded p-3 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        <strong className="text-gray-700">Sr. No.:</strong>
        <span className="">
          {prescriptions && prescriptions.length > 0 ? prescriptions.length + 1 : "1"}
        </span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <PrintableInput 
          type="date"
          className="bg-transparent border-none outline-none  p-0 h-auto flex-1 hover:bg-gray-400"
          value={patient.date}
    ...
    <div className="border border-gray-300 rounded p-3 flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <PrintableInput 
          className="flex-1 bg-transparent border-b border-gray-300 focus:border-teal-500 outline-none px-1 "
          value={doctor.name}
          onChange={(e: any) => handlePatientChange("doctorName", e.target.value)}
  ...
  {/* Second Row: Single box for Address */}
    ...
    <div className="flex items-center gap-2">
      ...
      />
            ...
            <div className="pointer-events-auto">
                <p className="font-bold text-gray-900 text-sm mt-4">Advice / Lifestyle Modifications:</p>
                <PrintableTextarea 
                    className="text-sm "
                    minHeight="3rem"
                    value={notes.advice}
            ...
            {/* Footer */}
            <div className="mt-auto pt-8 flex flex-col items-end pointer-events-auto bg-black text-white">
    <div className="w-48 border-b border-gray-600 mb-2"></div>
    <p className="text-sm font-bold text-white mr-4">{doctor.name}(Signature)</p>
    
    <div className="w-full border-t border-gray-600 mt-8 pt-2 text-center">
        <p className="text-xs text-gray-300">This digital prescription is generated by <strong className="text-white">ClinicPro+</strong> software.</p>
    </div>
</div>
