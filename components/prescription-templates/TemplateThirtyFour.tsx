...
const TemplateThirtyFour: React.FC<PrescriptionTemplateProps> = ({
  doctor,
  patient,
  notes,
  rows,
...
<div id="prescription-pad-content" className="relative flex flex-col min-h-[1123px] p-8 pointer-events-none z-0 border-8 border-green-900"
   style={{  fontFamily: fontStyle, color: textColor, transform: `scale(${fontSize / 100})`, transformOrigin: 'top left', width: `${100 / (fontSize / 100)}%` }}
>
 
      
   {/* Header */}
  <header className="flex justify-between items-start border-b-4 border-green-900 bg-white text-gray-100 text-start pt-8 pb-4 px-10 relative"
  style={{ borderBottom: `4px solid green `, borderImage: `${headerColor} 1` }}
> 
  {/* Text content */}
  <div className="flex-1">
    <h4 className="text-xl-1 text-green-900 font-bold mb-0 uppercase"
    ...
    <p className="text-sm-1 text-green-900 mb-1">{doctor.speciality} | Reg No: <strong>{doctor.regNo}</strong></p>
    <p className="text-sm-1 text-green-900 mb-0">{doctor.address} | Phone: {doctor.phone}</p>
  </div>

  {/* Logo */}
  <div className="w-16 h-16 flex items-center justify-center text-white font-bold text-xl overflow-hidden ml-4"
    style={{
...
{/* Patient Info Grid - Matching PDF table-like layout */}
     <section className="grid grid-cols-1 bg-gray-50 gap-y-1 gap-x-3 text-sm-1  -my-3 border-b-0 border-orange-500 pb-4 mb-8 py-8  pointer-events-auto">
     {/* First Row */}
