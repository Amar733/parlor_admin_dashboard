export const printPurchaseInvoice = async (
  ...
  try {
    const response = await authFetch(`/api/purchase/${purchaseId}/invoice`);
...
const generateInvoiceHtml = (data: any, printType: 'a4' | 'thermal', getAssetUrl: (url: string) => string) => {
  const { company, invoice, vendor, items, totals, notes } = data;
...
<body>
    ...
    <div class="invoice-info">
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span>${invoice.vendorBillNo}</span>
            </div>
            <div class="info-row">
                <span>Vendor Bill Date:</span>
                <span>${new Date(invoice.vendorBillDate).toLocaleDateString()}</span>
            </div>
        </div>
    ...
    <table class="items-table">
        <thead>
            <tr>
                ...
                <th>Rate</th>
                ${isA4 ? '<th>Amount</th>' : ''}
                ${isA4 ? '<th>CGST</th><th>SGST</th><th>IGST</th>' : '<th>Tax</th>'}
                <th>Total</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                ${isA4 ? `<td class="text-center">${item.storeName}</td>` : ''}
                ${isA4 ? `<td class="text-center">${item.hsnSac}</td>` : ''}
                <td class="text-center">${item.qty}</td>
                <td class="text-right">₹${item.rate.toFixed(2)}</td>
                ${isA4 ? `<td class="text-right">₹${item.subtotal.toFixed(2)}</td>` : ''}
                ${isA4 ? `
                <td class="text-right">${item.cgstPercent}%</td>
                <td class="text-right">${item.sgstPercent}%</td>
                <td class="text-right">${item.igstPercent}%</td>
                ` : `
                <td class="text-right">${item.cgstPercent + item.sgstPercent + item.igstPercent}%</td>
                `}
    ...
    <div class="totals-section">
        ...
        ${(totals.debitNoteAmount || 0) > 0 ? `
        <div class="total-row">
            <span>Debit Note:</span>
            <span>₹${(totals.debitNoteAmount || 0).toFixed(2)}</span>
        </div>` : ''}
        ${(totals.tds || 0) > 0 ? `
        <div class="total-row">
            <span>TDS:</span>
